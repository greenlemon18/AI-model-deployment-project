...
static void nms_sorted_bboxes(const std::vector<Object>& faceobjects, std::vector<int>& picked, float nms_threshold)
{
    ...
    int index = 0;
    float maxProb = faceobjects[0].prob;
...
static int detect_yolov5(const cv::Mat& bgr, std::vector<Object>& objects, float **output)
{
    ...
    fprintf(stderr, "[DEBUG] Checking 3 output heads...\n");
    for (int head = 0; head < 3; head++) {
        int h = (head == 0) ? 80 : (head == 1) ? 40 : 20;
        int step = 1 + 5;  // 6
        int num = 3 * h * h;
        float vmin = 999, vmax = -999, sum = 0;
        for (int i = 0; i < num * step; i++) {
            float v = output[head][i];
            if (v < vmin) vmin = v;
            if (v > vmax) vmax = v;
            sum += v;
        }
        fprintf(stderr, "[DEBUG] output[%d]: %d floats, range [%.4f, %.4f], avg %.4f\n",
                head, num * step, vmin, vmax, sum / (num * step));
        fprintf(stderr, "[DEBUG] output[%d] first 18: ", head);
        for (int i = 0; i < 18 && i < num * step; i++)
            fprintf(stderr, "%.4f ", output[head][i]);
        fprintf(stderr, "\n");
    }
    ...
    // 在 generate_proposals 调用前加
    for (int head = 0; head < 3; head++) {
        int h_size = (head == 0) ? 80 : (head == 1) ? 40 : 20;
        int step = 1 + 5;  // 6
        int total = 3 * h_size * h_size;
        float obj_max = -999, obj_sum = 0;
        for (int i = 0; i < total; i++) {
            float v = output[head][i * step + 4];  // obj at position 4
            if (v > obj_max) obj_max = v;
            obj_sum += v;
        }
        fprintf(stderr, "[OBJ] head %d: max=%.4f avg=%.4f\n", head, obj_max, obj_sum / total);
    }
