...
unsigned char *yolov5_pre_process(const char* imagepath, unsigned int *file_size)
{
    ...
    *file_size = img_size * sizeof(uint8_t);
    ...
    // 在 yolov5_pre_process 函数 return tensorData 前加
    {
        int in_hw = letterbox_rows * letterbox_cols;
        uint8_t vmin_r = 255, vmax_r = 0;
        uint8_t vmin_g = 255, vmax_g = 0;
        uint8_t vmin_b = 255, vmax_b = 0;
        for (int i = 0; i < in_hw; i++) {
            uint8_t r = tensorData[0 * in_hw + i];
            uint8_t g = tensorData[1 * in_hw + i];
            uint8_t b = tensorData[2 * in_hw + i];
            if (r < vmin_r) vmin_r = r; if (r > vmax_r) vmax_r = r;
            if (g < vmin_g) vmin_g = g; if (g > vmax_g) vmax_g = g;
            if (b < vmin_b) vmin_b = b; if (b > vmax_b) vmax_b = b;
        }
        fprintf(stderr, "[PRE-DEBUG] input range: R[%u,%u] G[%u,%u] B[%u,%u]\n",
                vmin_r, vmax_r, vmin_g, vmax_g, vmin_b, vmax_b);
        fprintf(stderr, "[PRE-DEBUG] first 10 pixels RGB: ");
        for (int i = 0; i < 10; i++) {
            fprintf(stderr, "(%u,%u,%u) ",
                tensorData[0 * in_hw + i],
                tensorData[1 * in_hw + i],
                tensorData[2 * in_hw + i]);
        }
        fprintf(stderr, "\n");
    }
